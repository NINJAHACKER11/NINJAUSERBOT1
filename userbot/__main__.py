...
from userbot import LOGS, NINJAversion, bot
from userbot.Config import Config
from userbot.helpers.runner import reload_NINJABOT
from userbot.start import abuses, addons, assistants, hekp, install, module, spams

l1 = Config.COMMAND_HAND_LER
l2 = Config.SUDO_COMMAND_HAND_LER
NINJA_PIC = "https://telegra.ph/file/e753315316673cff51085.mp4"
...
if len(sys.argv) not in (1, 3, 4):
    ...
    try:
        if Config.BOT_USERNAME is not None:
            ...
            print("♥️ Starting NINJABOT ♥️")
            bot.loop.run_until_complete(add_bot(Config.BOT_USERNAME))
            LOGS.info("🥇🔥 NINJABOT Startup Completed 🔥🥇")
        else:
            bot.start()
    except Exception as e:
...
async def killer():
    ...
    Cute_panda_01 = bot.uid
    legd_mention = f"[{NINJA_USER}](tg://user?id={Cute_panda_01})"
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            ...
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/officialninjauserbot"),
                    Button.url(" Updates ", "https://t.me/officialninjabotsupport"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer(
            "Reloading NINJAUSERBOT... Wait for few seconds...", cache_time=0, alert=True
        )
        await reload_NINJABOT()
    else:
        await event.answer(
...
async def users(event):
    if event.query.user_id == bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=".set var <varname> <value> ex:- .set var ALIVE_NAME NINJABOY \n\n To Know All Var Go Back And Click On All Var",
            buttons=[
                [custom.Button.inline("Back", data="osg")],
            ],
        )
...
║┣⪼ Group - @officialninjauserbot
║┣⪼ CREATOR - @Cute_panda_01
║┣⪼ NINJABOT - {NINJAversion}
║┣⪼ ✨ 『🔱N I N J A  🔱』𝐔𝐬𝐞𝐫𝐛𝐨𝐭✨
║╰━━━━━━━━━━━━━━━➣
╚══════════════════❍⊱"""
)
...
bot.loop.run_until_complete(ninjas())
