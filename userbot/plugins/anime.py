...
async def formatJSON(outData):
    ...
    if "errors" in res:
        ...
        # https://t.me/officialninjauserbot/19496
...
async def nope(lege):
    Ninja = lege.pattern_match.group(1)
    if not Ninja:
        if lege.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(Ninja))}")
...
async def nope(lege_):
    Ninja = lege_.pattern_match.group(1)
    if not Ninja:
        if lege_.is_reply:
            (await lege_.get_reply_message()).message
        else:
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(Ninja))}")
...
async def nope(lege_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(Ninja))}")
