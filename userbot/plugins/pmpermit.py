...
WARN_PIC = Config.PM_PIC or "https://te.legra.ph/file/1fef0a4acf69bfb85c511.jpg"
...
NINJA_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
NINJA_FIRST = (
    "**🔥 ninjaBo† Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that "
    "{} is currently unavailable.\nThis is an automated message.\n\n"
    "{}\n\n**{}Please Choose Why You Are Here!!**".format(
        Ninja_mention, CSTM_PMP, max_flood
    )
)
...
"""
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        if PM_WARNS[chat_id] == Config.MAX_FLOOD_IN_PM:
            r = await event.reply(NINJA_ZERO)
            await asyncio.sleep(3)
        ...
        Ninja_ = await tap[0].click(event.chat_id)
        PM_WARNS[chat_id] += 1
        chat_id = chat_id
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
        PREV_REPLY_MESSAGE[chat_id] = Ninja_
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pm_sql.is_approved(chat.id):
            pm_sql.approve(
                chat.id,
                f"**My Boss iz here..{Ninja_mention}'s Its Ur Lucky day Nibba😏!!**",
            )
...
CmdHelp("pm_permit").add_command(
    ...
    "listapproved", None, "Sends the list of all users approved by ninjaBot"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
