...
async def nope(aura):
    NINJA = aura.pattern_match.group(1)
    if not NINJA:
        if aura.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(NINJA))}")
...
async def _(event):
    ...
    thumb_name = f"thumb{The_NinjaHacker}.jpg"
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [NINJABOT](https://t.me/NINJASupport)",
            )
            await event.delete()
        except YouBlockedUserError:
