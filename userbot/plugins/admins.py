...
async def promote(promt):
    ...
    NINJAevent = await edit_or_reply(promt, "Promoting...")
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    NINJAevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
...
async def ban(bon):
    ...
    if not user:
        return
    NINJAevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await NINJAevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await NINJAevent.edit(f"{str(user.id)} ιѕ ϐαииє∂ !!\nяєαѕοи: {reason}")
    else:
        await NINJAevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    NINJAevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await NINJAevent.edit("υиϐαииє∂ ѕυϲϲєѕѕƒυℓℓγ! Giving one more chance 😏")
        if BOTLOG:
        ...
        await NINJAevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    NINJAevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await NINJAevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        await NINJAevent.edit(
        ...
        await NINJAevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
