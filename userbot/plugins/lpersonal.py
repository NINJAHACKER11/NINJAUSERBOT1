...
OS = [
    ...
    "PLS DONT DISTURB HIM NINJA IS BUSY NOW WHEN HE COME BACK HE REPLY U",
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        event = await edit_or_reply(event, "NINJA")
        que[e] = []
        qeue = que.get(e)
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"『🔱 NINJA ™🔱』IS STARTING GAME")
    else:
        ...
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"NINJA {ALIVE_NAME}")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        queue = que.get(e)
        queue.pop(0)
        await event.edit(f"NINJA STOPED RAID {ALIVE_NAME}")


CmdHelp("lpersonal").add_command(
    "lstarts", None, "Reply to him or her to start Ninja personal file"
).add_command("lstops", None, "Reply To her Ya him To stop Ninja personal file").add()
