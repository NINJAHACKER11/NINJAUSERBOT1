...
from NINJABOT.utils import admin_cmd, eor, sudo_cmd
from userbot import HEROKU_APP, NINJAversion, bot
...
async def _(event):
    ...
    await event.edit("Rєϐοοτιиg **[ ███ ]** ...\nωαιτ ƒєω мιиυτєѕ☢️")
    await event.edit(
        f"Rebooted ninjaBot - {NINJAversion} **[ ⚡ ]** ...\n**Type `.ping` or `.Ninja` after 5min to check if I am working**"
    )
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(
        "**[ ! ]** `⚰️Turning off bot now ... Manually turn me on later or follow step of update in @ninja_Userbot` ಠ_ಠ"
    )
...
async def rel(event):
    await eor(event, "Reloading NINJABOT... Wait for few seconds...")
