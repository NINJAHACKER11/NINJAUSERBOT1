...
async def _(event):
    ...
    Ninja = await eor(event, "**Black Magics Started... Lets See**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
            await asyncio.sleep(0.5)
    await Ninja.edit("**Bleck Magik Done...🤣**")
...
async def _(event):
    ...
    Ninja = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
            await asyncio.sleep(0.5)
    await Ninja.edit("**Bleck Magik Completed...**")
    await bot.send_message(
        Config.LOGGER_ID,
...
async def _(event):
    ...
    q = 0
    r = 0
    Ninja = await edit_or_reply(event, "Searching Participant Lists.")
    async for i in bot.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(Ninja, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await Ninja.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
