...
async def install(event):
    ...
    b = 1
    owo = event.text[9:]
    Ninja = await eor(event, "__Installing.__")
        ...
        try:
            ...
            try:
                if EVAL == "ON":
                    ...
                    return await eod(
                        Ninja, "First Turn ON Eval CMD = `.set var EVAL ON`"
                    )
            except Exception as e:
                await eod(Ninja, f"{e}")
                return os.remove(download_file_name)
        except Exception as e:
            await eod(Ninja, f"**Failed to Install** \n`Error`\n{str(e)}")
...
async def install(event):
    ...
    _o = o.split("\n")
    o = "\n".join(_o)
    OUTPUT = f"♥️List Of Plugins In ninjaẞø✞︎ 🇮🇳 :- \n\n{o}\n\n<><><><><><><><><><><><><><><><><><><><><><><><>\nHELP:- If you want to know the commands for a plugin, do :- \n.plinfo <plugin name> without the < > brackets. \nJoin https://t.me/officialninjauserbot for help."
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "cmnds_list.text"
            NINJA_file = await bot.send_file(
                event.chat_id,
                out_file,
                force_document=True,
                allow_cache=False,
            ...
            await edit_or_reply(
                NINJA_file,
                f"**Output Too Large. This is the file for the list of plugins in ninjaẞø✞︎**.\n\nBY :- **{DEFAULTUSER}**",
            )
            await event.delete()
...
CmdHelp("core").add_command(
    ...
    "cmds", None, "Gives out the list of modules in NINJABOT."
).add_warning(
