...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "ninjaary NINJABOT"
DEL_TIME_OUT = 60
...
async def _(event):
    if event.fwd_from:
        return
    NINJA = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    ...
    await NINJA.edit(f"Auto Name has been started my Master")
