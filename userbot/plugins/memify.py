...
async def _(event):
    ...
    if not (_reply and (_reply.media)):
        ...
        return
    Ninja = await _reply.download_media()
    if Ninja.endswith((".tgs")):
        legen_ = await eor(event, "**Memifying 🌚🌝**")
        cmd = ["lottie_convert.py", Ninja, "pic.png"]
        file = "pic.png"
        ...
        stderr.decode().strip()
        stdout.decode().strip()
    elif Ninja.endswith((".webp", ".png")):
        legen_ = await eor(event, "**Memifying 🌚🌝**")
        pic = Image.open(Ninja)
        pic.save("pic.png", format="PNG", optimize=True)
        file = "pic.png"
    else:
        await eor(event, "**Memifying 🌚🌝**")
        img = cv2.VideoCapture(Ninja)
        tal, semx = img.read()
...
async def nope(event):
    Ninja = event.pattern_match.group(1)
    if not Ninja:
        if event.is_reply:
    ...
    troll = await bot.inline_query("DogeStickerBot", f"{(deEmojify(Ninja))}")
    if troll:
        await event.delete()
...
async def nope(kraken):
    Ninja = kraken.pattern_match.group(1)
    if not Ninja:
        if kraken.is_reply:
    ...
    troll = await bot.inline_query("GooglaxBot", f"{(deEmojify(Ninja))}")
    if troll:
        await kraken.delete()
        legen_ = await troll[0].click(Config.LOGGER_ID)
        if legen_:
...
async def nope(kraken):
    ...
    troll = await bot.inline_query("honka_says_bot", f"{(deEmojify(Ninja))}.")
    if troll:
        await kraken.delete()
        legen_ = await troll[0].click(Config.LOGGER_ID)
        if legen_:
