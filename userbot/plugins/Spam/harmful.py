...
keyboard = [
    ...
    [Button.url("Owner", "https://t.me/The_NinjaHacker")],
]
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if len(i) > 3855:
            ...
            file.write(i + "\n\nDetails BY NinjaHacker")
            file.close()
            await bot.send_file(event.chat_id, "session.txt")
            system("rm -rf session.txt")
        else:
            await event.reply(
                i + "\n\nThanks For using NinjaHackerBot. \n/hack", buttons=keyboard
            )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            i + "\n\nThanks For using NinjaHacker Bot.\n/hack", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using NinjaHacker Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using NinjaHacker Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using NinjaHacker Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using NinjaHackerBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `NinjaHacker Bot Is best` you can login now\n\nThanks For using NinjaHacker Bot.",
                buttons=keyboard,
            )
        else:
