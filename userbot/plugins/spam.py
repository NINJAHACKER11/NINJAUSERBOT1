...
async def bigspam(NINJA):
    if not NINJA.text[0].isalpha() and NINJA.text[0] not in ("/", "#", "@", "!"):
        NINJA_msg = NINJA.text
        NINJABOT_count = int(NINJA_msg[9:13])
        NINJA_spam = str(NINJA.text[13:])
        for i in range(1, NINJABOT_count):
            await NINJA.respond(NINJA_spam)
        await NINJA.delete()
        if LOGGER:
            await NINJA.client.send_message(
    ...
    if lg_id:
        if event.is_private:
            ...
            await event.client.send_message(
                lg_id,
                "#WSPAM\n"
                + f"Word Spam was executed successfully in {Ninja_mention} chat with : `{message}`",
            )
