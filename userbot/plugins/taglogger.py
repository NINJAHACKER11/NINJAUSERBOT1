...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        Ninja = await event.client.get_entity(event.sender_id)
        if Ninja.bot or Ninja.verified or Ninja.support:
            return

        Ninjam = f"[{get_display_name(Ninja)}](tg://user?id={Ninja.id})"
