...
async def _(event):
    ...
    NINJAuser = await event.client.get_me()
    if NINJAuser.username:
        curruser = NINJAuser.username
    else:
        curruser = "@Official_ninjaBot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    if EVAL == "ON":
        ...
        NINJAevent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec",
    "<command>",
    "Execute a Terminal command on NINJABOT server and shows details",
).add_command(
