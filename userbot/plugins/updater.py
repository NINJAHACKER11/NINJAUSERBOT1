...
BOT_IS_UP_TO_DATE = "**The NINJABOT** is up-to-date sir."
NEW_BOT_UP_DATE_FOUND = (
    "new update found for {branch_name}\n"
    "changelog: \n\n{changelog}\n"
    "updating your NINJABOT ..."
)
NEW_UP_DATE_FOUND = (
    "New update found for {branch_name}\n" "`updating your NINJABOT...`"
)
...
Ninjabot_info = "https://raw.githubusercontent.com/The-ninjaBot/NINJAUSERBOT/ninjaBot/Ninjaboy-info.json"
...
async def Ninja_info(Ninjabot_info):
    infos = requests.get(Ninjabot_info).json()
    _version = infos["NINJABOT-INFO"]["version"]
    _release = infos["NINJABOT-INFO"]["release-date"]
    _branch = infos["NINJABOT-INFO"]["branch"]
    _author = infos["NINJABOT-INFO"]["author"]
    _auturl = infos["NINJABOT-INFO"]["author-url"]
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"🌚 Do `.update build` to update your **ninjaẞø†** !!"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/NINJA-OS/NINJABOT"
    os.chdir("/app")
    git_Ninja = f"rm -rf .git"
    try:
        await runcmd(git_Ninja)
    except BaseException:
        pass
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    _version, _release, _branch, _author, _auturl = await Ninja_info(Ninjabot_info)
    await event.edit(
        f"<b><i>NINJABOT Docker Build In Progress !!</b></i> \n\n<b><i><u>Update Information :</b></i></u> \n<b>• Branch :</b> {_branch} \n<b>• Release Date :</b> {_release} \n<b>• Version :</b> {_version} \n<b>• Author :</b> <a href='{_auturl}'>{_author}</a>",
        link_preview=False,
        parse_mode="HTML",
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(
            f"**Your NINJABOT Is UpToDate**\n\n**Version :**  __{NINJAversion}__\n**Oɯɳҽɾ :**  {Ninja_mention}"
        )
    else:
        await event.edit(
