...
from NINJABOT.utils import admin_cmd, edit_or_reply, eor, sudo_cmd
...
async def get_users(event):
    ...
    Ninja_chat = legen_.lower
    restricted = ["@ninja_Userbot", "@Official_ninjaBot"]
    Ninja = await eor(event, f"**Inviting members from** {legen_}")
    if Ninja_chat in restricted:
    ...
    if not sender.id == me.id:
        NINJA = await edit_or_reply(event, "`processing...`")
    else:
        NINJA = await edit_or_reply(event, "`processing...`")
    Ninja = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await NINJA.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await NINJA.edit(
        "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/officialninjauserbot)**\n\n`🔸Inviting Users.......`"
    )
    async for user in event.client.iter_participants(Ninja.full_chat.id):
        try:
            ...
            error = str(e)
            f = f + 1
    return await NINJA.edit(
        f"[τєямנиαℓ ƒιиιѕнє∂](https://t.me/officialninjauserbot) \n\n🔸 Sυϲϲєѕѕƒυℓℓγ ιиνιτє∂ `{s}` ρєορℓє \n⚠️ ƒαιℓє∂ το ιиνιτє `{f}` ρєορℓє"
    )
...
async def get_users(event):
    ...
    await NINJA.edit("**TerminalStatus**\n\n`Collecting Users.......`")
