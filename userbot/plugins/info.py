...
<b>⚡ <a href='https://t.me/officialninjauserbot'>From DataBase of NINJABOT</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
async def info(event):
    if event.fwd_from:
        return
    Ninja = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await Ninja.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(Ninja, "`An unexpected error has occurred.`")
    return
