...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of NINJABOT`")
        return
    NINJABOT = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await NINJABOT.edit("Committing to Github....")
        await git_commit(downloaded_file_name, NINJABOT)


async def git_commit(file_name, NINJABOT):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await NINJABOT.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await NINJABOT.edit("Cannot Upload Plugin")
    else:
        return await NINJABOT.edit("`Committed Suicide`")
