...
async def _(Ninjaevent):
    i = 0
    await Ninjaevent.get_sender()
    me = await Ninjaevent.client.get_me()
    Ninja = await eor(Ninjaevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await Ninjaevent.get_chat()
    if Ninjaevent.is_private:
        user = Ninjaevent.chat
        rank = Ninjaevent.pattern_match.group(1)
    else:
        Ninjaevent.chat.title
    try:
        user, rank = await get_full_user(Ninjaevent)
    except:
        pass
    if me == user:
        await Ninja.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "ℓεɠεɳ∂"
    except:
        return await Ninja.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await Ninjaevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await Ninjaevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await Ninja.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await Ninja.edit(f"**Reply to a user !!**")
    await Ninja.edit(
...
async def _(Ninjaevent):
    ...
    await Ninjaevent.get_sender()
    me = await Ninjaevent.client.get_me()
    Ninja = await eor(Ninjaevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await Ninjaevent.get_chat()
    if Ninjaevent.is_private:
    ...
    try:
        ...
        pass
    if me == user:
        await Ninja.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "Ninja"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await Ninjaevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await Ninja.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await Ninja.edit(f"**Reply to a user !!**")
...
async def _(event):
    Ninja = await eor(event, "`Gbanning...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(
            Ninja, "**To gban a user i need a userid or reply to his/her message!!**"
        )
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == The_NinjaHacker:
        return await eod(Ninja, "🥴 **Nashe me hai kya lawde ‽**")
    ...
    if a is not None:
        ...
        gbpic = gban_pic
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **Gbanned** By {Ninja_mention} \n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {Ninja_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(Ninja, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eor(Ninja, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                await event.client.edit_permissions(
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            Ninja = int(hel)
            try:
                tity = await event.client.get_entity(Ninja)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={Ninja}) (`{Ninja}`)\n"
    else:
...
async def gkick(event):
    Ninja = await eor(event, "`Kicking globally...`")
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(Ninja, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == The_NinjaHacker:
        return await eod(Ninja, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eor(Ninja, "**😪 I'm not going to gkick my developer!!**")
