...
from NINJABOT.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(NINJA):
    if NINJA.fwd_from:
        return
    reply_to_id = NINJA.message.id
    if NINJA.reply_to_msg_id:
        reply_to_id = NINJA.reply_to_msg_id
    event = await edit_or_reply(NINJA, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await NINJA.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await NINJA.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    NINJAreply = await event.get_reply_message()
    if not NINJAreply or not NINJAreply.media or not NINJAreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if NINJAreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    NINJAevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, NINJAreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await NINJAevent.edit("`This file is not supported`")
            NINJAresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            NINJAfile = Path(
                await event.client.download_media(NINJAresponse, "./temp/")
            )
            NINJAgif = Path(await unzip(NINJAfile))
            Ninja = await event.client.send_file(
                event.chat_id,
                NINJAgif,
            ...
            for files in (NINJAgif, NINJAfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await NINJAevent.edit("Unblock @tgstogifbot")
            return
