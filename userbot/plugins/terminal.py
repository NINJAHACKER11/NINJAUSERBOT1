...
NINJA = str(ALIVE_NAME) if ALIVE_NAME else "Du"
...
async def dc(event):
    await event.edit(f"{NINJA}: Running Terminal.....")
    ...
    if cmd in ("userbot.session", "env", "printenv"):
        return await event.edit(f"{NINJA}: Privacy Error, This command not permitted")
    ...
    await event.client.send_file(
        ...
        "term.txt",
        reply_to=event.id,
        caption=f"{NINJA}: Output too large, sending as file",
    )
    os.remove("term.txt")
    return
