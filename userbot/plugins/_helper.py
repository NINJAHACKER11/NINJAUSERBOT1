...
async def repo(event):
    try:
        Ninja = await bot.inline_query(botname, "repo")
...
async def _(event):
    ...
    if tgbotusername is not None:
        try:
            results = await event.client.inline_query(tgbotusername, "Ninjabot_help")
            ...
            async with bot.conversation(chat) as conv:
                try:
                    ...
                    return await Ninja.edit("Unblock @Botfather first.")
                await Ninja.edit(
...
async def yardim(event):
    ...
    if tgbotusername is not None or NINJA_input == "text":
        results = await event.client.inline_query(tgbotusername, "Ninjabot_help")
        await results[0].click(
            event.chat_id, reply_to=event.reply_to_msg_id, hide_via=True
        )
