...
async def fs(event):
    ...
    if not await is_admin(event, Ninjabooy):
    ...
    if args in ("off", "no", "disable"):
        ...
        rip = await check_him(channel, Ninjabooy)
        if rip is False:
            return await event.reply(
    ...
    if chat_db:
        try:
            ...
            if rip is False:
                ...
                Ninjaboy = Config.BOT_USERNAME
                await bot(
                    EditBannedRequest(event.chat_id, event.sender_id, MUTE_RIGHTS)
                )
                response = await bot.inline_query(Ninjaboy, "pm_warn")
...
@bot.on(events.InlineQuery(pattern="Ninjabooy"))
async def NOOBBOY(event):
    global rk
    event.builder
    Ninjaproboy = [[Button.inline("Unmute Me 😊", data="LeGeNd")]]
    NOOBBOYOP = Ninjaboy.article(
        title="FORCE SUBSCRIBE", text="fsub", buttons=Ninjaproboy
    )
