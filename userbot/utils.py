...
from userbot import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, Ninja, NINJABOT, bot
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("NINJAUSERBOT ~ " + shortname)
    else:
        import userbot.utils
        ...
        mod.delete_NINJA = delete_NINJA
        mod.eod = delete_NINJA
        mod.admin_cmd = admin_cmd
        mod.ninja_cmd = admin_cmd
        mod.sudo_cmd = sudo_cmd
        # support for NINJABOT originals
        sys.modules["NINJABOT.utils"] = userbot.utils
        sys.modules["NINJABOT"] = userbot
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        name = "userbot.plugins.NINJABOT-Addons.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["NINJABOT-Addons." + shortname] = mod
...
def load_abuse(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("NINJABOT-Abuse ~ " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                NINJAreg = "^" + Config.COMMAND_HAND_LER
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                NINJAreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_NINJA(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/Cute_panda_01)"
