...
lnbot = TelegramClient("NINJABOY", API_ID, API_HASH).start(bot_token=token)


class NINJABOY(TelegramClient):
...
if Config.NINJA_STRING:
    session_name = str(Config.NINJA_STRING)
    sweetie = NINJABOY(session_name, Config.APP_ID, Config.API_HASH)
else:
